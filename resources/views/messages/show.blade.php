...
@section('contenido')
    ...
    <div class="flex flex-col max-w-xl px-8 py-4 mx-auto bg-white rounded shadow h-96 dark:bg-slate-800">
        <p class="flex-1 leading-normal text-slate-600 dark:text-slate-400">Enviado por: {{$message->present()->userName()}}</p>
        <p class="flex-1 leading-normal text-slate-600 dark:text-slate-400">Email: {{$message->present()->userEmail()}}</p>
