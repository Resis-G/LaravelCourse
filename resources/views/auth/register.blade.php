    ...
    <form class="max-w-xl px-8 py-4 mx-auto bg-white rounded shadow dark:bg-slate-800" method="POST" action="{{ route('register.store') }}">
        ...
        @foreach ($roles as $role)
            <label class="flex flex-row  mt-4 " for="roles">
                ...
                {!!$errors->first('roles','<span class=errors>:message</span>')!!}

            </label>
        @endforeach

        <div class="flex items-center justify-between mt-4">
            <input class="inline-flex items-center px-4 py-2 text-xs font-semibold tracking-widest text-center text-white uppercase transition duration-150 ease-in-out border border-2 border-transparent rounded-md dark:text-sky-200 bg-sky-800 hover:bg-sky-700 active:bg-sky-700 focus:outline-none focus:border-sky-500" type="submit" value="Registrar">
        </div>
    </form>
